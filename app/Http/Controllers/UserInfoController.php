...
class UserInfoController extends Controller
{
	public static function oauthZapierSubmit(Request $request){
		...
		if( UserInfoController::authenticateUser($email, $password) != 0){
	...
	public static function GetProfileDataFromZapier(Request $request){
		...
		for( $i = 0; $i < count($profiles); $i++){
			...
			$profile->lastname = $curProfile->LastName;
			$profile->headline = $curProfile->Headline;
			$profile->location = $curProfile->Location;
			$profile->url = $curProfile->Url;
			$profile->email = $curProfile->Email;
			$profile->imgurl = $curProfile->ImgUrl;
			$profile->phonenumber = $curProfile->PhoneNumber;
			$profile->lastjob = $curProfile->LastJob;
			$profile->twitter = $curProfile->Twitter;
			$profile->site = $curProfile->Site;
			$profile->tag = $curProfile->Tag;
			array_push( $retVal, $profile);
			array_push($ids, $curProfile->Id);
		}
		DB::update('update profiles set ZapTag = 1 where Id in (?)', implode(',', $ids));
